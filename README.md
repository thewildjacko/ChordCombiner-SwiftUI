...
The same `Note` can be spelled with a different `KeyName` depending on other elements of its identity. For example, two notes can share the same `NoteNumber` of **`.three`**, but if one has a `Degree` of **`.minor3rd`** and the other has a `Degree` of **`.sharp9th`**, the first will display as **"E♭"**, while the second will display as **"D♯"**. Another example: two `Note`s have both a **`NoteNumber`** of **`.ten`** and a `Degree` of **`.major3rd`**; however, one's **`Accidental`** is **`.flat`** and the other's is **`.sharp`**. The first will display as **"B♭"** and the second will display as **"A♯"**.
