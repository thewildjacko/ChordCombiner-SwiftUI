...
The `Degree` of the `Note` can change the `KeyName`. For example, two notes could share the same `NoteNumber` of **`.three`**, but one has a `Degree` of **`.minor3rd`** and the other has a `Degree` of **`.sharp9th`**. The first note will display as **"E♭"**, while the second will display as **"D♯"**
