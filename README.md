...
The `Degree` of the `Note` can change the `KeyName`. For example, two notes can share the same `NoteNumber` of **`.three`**, but if one has a `Degree` of **`.minor3rd`** and the other has a `Degree` of **`.sharp9th`**, the first will display as **"E♭"**, while the second will display as **"D♯"**
